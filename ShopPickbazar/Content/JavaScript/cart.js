...
function renderCart() {
    ...
    if (cart.length > 0) {
        ...
        cart.forEach(item => {
            ...
            listItem.innerHTML = `<div class="flex items-center  border-b border-solid border-border-200 border-opacity-75 px-4 py-4 text-sm sm:px-6" style="opacity: 1; transform: none; transform-origin: 50% 50% 0px;">
        ...
        cartListElement.innerHTML = `<div class="flex h-full flex-col items-center justify-center" style="opacity: 1;"><svg width="140"
                    ...
                    <h4 class="mt-6 text-base font-semibold">Giỏ hàng trống</h4>
                </div>`;
    }
    for (item of totalPriceElem) {
        item.textContent = totalPrice.toLocaleString("vi-VN", {
            style: "currency",
            currency: "VND"
        });
    }
    for (item of productQuantity) {
        item.textContent = `${cart.length} Sản phẩm`;
    }
    setupRemoveButtons();
